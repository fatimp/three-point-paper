...
\begin{table*}[!htp]
  ...
  \begin{tabular}{|c|c|c|c|c|c|}
    \hline
    Sample name & Image type & Dimensions (pixels) & Image resolution ($\mu m / px$)
    & $C_{0.5}$ & Comment \\
    \hline
    Sandstone & XCT &  $500 \times 500 \times 500$ & 2.25 & 0.9703 & \\
    Soil & XCT & $500 \times 500 \times 500$ & 0.72 & 0.9431 & Digitally
    magnified \\
...
the criterion $C_{0.5} > 0.97$ holds (the two sandstones and the soil). Images
...
In this section we measure execution times of our algorithms for random
3-dimensional arrays of bits (i.e. each element can be either 0 or 1). The
arrays have equal number of elements along each direction therefore
representing cubes of 2-phase porous media. The number of voxels $N$ in each
cube increases from $1.25 \cdot 10^5$ to $2.7 \cdot 10^7$ linearly. Times of
execution are measured for each function executed on a CPU and a GPU. Hardware
and software components used for testing are listed in \cref{tab:machine}. Each
computation of a correlation function is equivalent to testing $N^{5/3}$ pattern
triangles thrown into the sample.
\begin{table*}[!htp]
  \centering
  \begin{tabular}{|c|c|}
...
\begin{figure*}[tp]
  ...
  \label{fig:timings}
\end{figure*}
The results are on \cref{fig:timings}. It can be observed that the time of
execution is a power function of number of voxels. GPU gives a
10x-100x performance boost depending on the correlation function.
