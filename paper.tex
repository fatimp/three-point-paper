...
is a total number of clusters. A cluster is a subset of $A_n$ so that any two
...
\begin{align}
  ...
  F_{ssv}^n(x_1, x_2) &= \langle M_n(x) M_{(void)}(x + x_1) I_{(void)}(x + x_2) \rangle \\
...
$\mu m^{-2}$ and $\mu m^{-1}$ or other convenient dimensionless units depending
on the problem to be solved).  These functions describe the interface of the
subset $A_n$ and its spatial configuration with respect to $A_v$.
...
To estimate the error introduced by rotation, consider a
$300 \times 300 \times 300$ sample of value noise \cref{fig:roterror-noise}
which is defined as $f(x, y, z) > T$ where $f$ is a value noise function and $T$
is a threshold. We produce images of the noise rotated around $Z$ axis by $\phi$
radians by discretizing an inequation $f(p(x'), p(y'), p(z')) > T$ where
\begin{equation*}
  \begin{aligned}
...
\begin{equation*}
  ...
  \end{aligned}
\end{equation*}
Therefore we assume the noise to be periodic with the period equal to
$2$ and the interface between phases is continuous. Discretizing for different
values of $\phi \in [0, \pi/4]$ we obtain the first dataset which contains the
best possible discretization of the rotated noise.

The second dataset is constructed by discretizing the noise with $\phi = 0$ and
then rotating the discretized image using nearest-neighbor interpolation.

We compute $S_3$ and $F_{sss}$ functions in periodic mode for the both datasets
and calculate pointwisely a relative error introduced by the rotation as
\begin{equation}
  \varepsilon = \frac{|F_1 - F_2|}{F_1}
  \label{eq:relerr}
\end{equation}
where $F_1$ is a correlation function for a sample from the first dataset and
$F_2$ is a correlation function for a sample from the second dataset. The we
calculate the maximal relative error for each $\phi$ as the maximum of all
pointwise errors for a given rotation angle.

Plots of maximal relative errors for functions $S_3$ and $F_{sss}$ are on
\cref{fig:roterror-error}. For the surface-surface-surface function we omit
values $F_{sss}(s_1, s_2)$ if either of its arguments is less than 10 voxels
because this function is undefined for either $s_1$ or $s_2$ being equal to zero
and our edge detection filter gives at least 7 voxel transition zone where the
correlation function is affected by this indeterminacy. The maximal error for
all $\phi$'s are 0.6\% for $S_3$ and 4.4\% for $F_{sss}$.
