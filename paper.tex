...
3-point CFs. The execution times for the same number of samples are orders of
...
The described algorithms are implemented in our \code{CorrelationFunctions.jl}
package written in Julia and have the same interface:
\begin{verbatim}
corrfn(input, phase, ps1, ps2)
\end{verbatim}
where \code{corrfn} stands for \code{s3} ($S_3$), \code{c3} ($C_3$),
\code{surf3} ($F_{sss}$) etc., \code{input} is the input array, \code{phase} is
...
Our implementation provides a predefined right triangle pattern where arguments
of a correlation function $x_1$ and $x_2$ are fixed to be parallel to one of the
axes $X$, $Y$ or $Z$ with restriction $x_1 \perp x_2$ (\cref{fig:pattern})
\cite{dimitrakopoulos2010high}. The output has dimensions $D_1 \times D_2$ where
$D_1$ and $D_2$ are dimensions of the input array along selected axes.
\highlight{I CANNOT UNDESTAND THIS SENTENCE: Note that shift operators with such
  $x_1$ and $x_2$ represent a 3-point function computed within a 3D domain, but
  along a predefined direction (see below).}
\begin{figure}[ht]
  \centering
...
If it is desired, a user can use a custom pattern more appropriate to the
situation.
...
pattern (\cref{fig:aniso}). This has such consequences as reduced accuracy of
computed functions due to necessity to interpolate the image during
rotation. Although we show in \cref{sec:rotqua} that even nearest-neighbor
...
but it can be mitigated by choosing a random pattern or rotating the
pattern. The rotations can be random as in the Malmir et al. algorithm, or it
can be a single rotation to align catheti of the sampling triangle with the
...
function which generates a right-angle triangle pattern. Computations are easily
...
correlation functions with different "n-pointness" is not known, it is seemingly
no much reason to increase the number of points. Nonetheless, developed
algorithms and code are universal enough to be easily extendable to any $n$
(with increasing computational and RAM burden). All results are available as
part of open source package \code{CorrelationFunctions.jl}, developed by our
group and freely available to the public with numerous Jupiter notebooks showing
examples of applications to assess 3-point correlation functions for 2D and 3D
images.
...
\section{Difference between rotating the input image or the sampling pattern}
\label{sec:rotrot}
...
\begin{figure*}[tp]
  \centering
  \subfigure[A sample of value noise with small pores (compared to a size of the
    sample, $300 \times 300 \times 300$)]{
